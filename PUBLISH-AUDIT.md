...
|---|---|---|---|---|---|---|
| Logic Arrows Launcher | This repository, current working tree, v1.0.7 dark UI polish, toolbar tooltip correction, WebView2 bind-focus recovery, stale-key reset and dark selection overlay cleanup | No separate launcher license declared | `src/`, `smoke/`, `assets/`, `tools/`, README, docs, platform folders | Reviewed before publish; no secrets found | `Microsoft.Web.WebView2` 1.0.4129.50 pinned in csproj | PASS |
| `.map` integration | `src/MapFileService.cs`, `src/MapBridgeScript.cs`, `src/LauncherForm.cs`; v1.0.7 UI-only dark bridge, toolbar tooltip styling and alt-tab bind-focus recovery | Launcher-owned format; embedded Logic Arrows data remains game-owned | JSON envelope validation, Base64 boundary, origin-checked WebView2 message bridge | Round-trip, invalid-format, adaptive TPS/theme, page focus, stale-key reset, dark selection overlay cleanup and v1.0.7 UI smoke tests PASS; JS syntax PASS | No new package added | PASS |
...
`LogicArrowsLauncher.exe` is a Windows x64 self-contained single-file publish for v1.0.7 dark UI polish, toolbar tooltip correction, WebView2 bind-focus recovery, stale-key reset and dark selection overlay cleanup.

SHA-256: `11b89be4448bd763b33d468aa962241f562d208d11e0af6f26a42679ac092cd3`

Size: 72,064,083 bytes.
