...
| Adaptive TPS/theme JS smoke | PASS: high-TPS budget, draw governor, low-TPS official path и 3 темы |
| Настройки и тема | Собрано; ручной Windows UI-тест Системная/Тёмная/Светлая нужен на целевой машине |
| Импорт из lobby maps tab | Собрано; ручной Windows UI-тест выбора файла и запуска нужен на целевой машине |
...
На большой карте создай или открой сложный механизм, включи максимальный TPS и посмотри на FPS/отзывчивость: тики должны продолжаться, а интерфейс не должен зависать из-за одного длинного кадра. Governor не обещает теоретические 6000 TPS на любой схеме; если схема тяжелее бюджета, фактический TPS будет снижаться ради сохранения FPS.

В настройках открой dropdown **«Тема»**, по очереди выбери **Системная**, **Тёмная**, **Светлая**, перейди в другую вкладку и обратно. Цвета должны меняться сразу, а выбранная тема сохраняться после перезапуска.
...
Затем проверь импорт: нажми **«Играть»**, в игровом лобби открой вкладку **«Карты»**, нажми карточку **«Импорт .map»** рядом с карточкой создания новой карты и выбери файл. После создания новой карты убедись, что схема загрузилась. Для экспорта открой меню карты, проверь ровный нижний ряд **«Назад в меню» + «Экспорт .map»**, выбери путь сохранения и затем импортируй полученный файл обратно через вкладку карт.
